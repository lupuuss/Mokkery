...
class MocksPropertiesTest {
    ...
    fun testGetPrimitiveExtension() {
        every { mock.run { 1.primitivePropertyExtension } } returns 1
        assertEquals(1, mock.run { 1.primitivePropertyExtension })
        verify { mock.run { 1.primitivePropertyExtension } }
    }

    @Test
    fun testSetPrimitiveExtension() {
        every { mock.run { 1.primitivePropertyExtension = 1 } } returns Unit
        mock.run { 1.primitivePropertyExtension = 1 }
        verify { mock.run { 1.primitivePropertyExtension = 1 } }
    }

    @Test
    fun testGetComplexExtension() {
        every { mock.run { ComplexType.complexPropertyExtension } } returns ComplexType.Companion
        assertEquals(ComplexType.Companion, mock.run { ComplexType.complexPropertyExtension })
        verify { mock.run { ComplexType.complexPropertyExtension } }
    }

    @Test
    fun testSetComplexExtension() {
        every { mock.run { ComplexType.complexPropertyExtension = ComplexType.Companion } } returns Unit
        mock.run { ComplexType.complexPropertyExtension = ComplexType.Companion }
        verify { mock.run { ComplexType.complexPropertyExtension = ComplexType.Companion } }
    }
}
