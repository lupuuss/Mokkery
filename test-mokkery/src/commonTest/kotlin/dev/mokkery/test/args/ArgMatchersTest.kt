...
private fun <T> ArgMatchersScope.isNull(): T = customMatcher { it == null }

private fun <T> ArgMatchersScope.customMatcher(block: (T) -> Boolean): T = matches { block(it) }

private fun <T> ArgMatchersScope.customCapture(
    capture: Capture<T>,
    @Matcher matcher: T = customMatcher { true }
) = capture(capture, matcher)

@OptIn(DelicateMokkeryApi::class)
private fun <T> ArgMatchersScope.customAnd(
    @Matcher first: T,
    @Matcher second: T,
...
private fun ArgMatchersScope.regularValueMatcher(): Int = 1

private fun ArgMatchersScope.anyOrEq(
    condition: Boolean,
    value: Int
): Int = if (condition) value else any()

@VarArgMatcherBuilder
private fun ArgMatchersScope.varargsIntAllEq(value: Int): IntArray = varargsIntAll { it == value }

@VarArgMatcherBuilder
private fun ArgMatchersScope.notVarargsIntAllEq(value: Int): IntArray = not(varargsIntAllEq(value))

private fun ArgMatchersScope.rawMatcher(arg: ArgMatcher<Int>): Int = matches(arg)
