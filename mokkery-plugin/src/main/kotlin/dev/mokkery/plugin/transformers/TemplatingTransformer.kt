...
class TemplatingTransformer(
    ...
    private fun IrBuilderWithScope.replaceTopTemplatingArg(
        ...
        return when {
            frontExpression == null -> irCallConstructor(expectDefaultMatcherConstructor) {
    ...
    private fun IrBuilderWithScope.replaceVararg(expression: IrExpression): IrExpression {
        if (expression !is IrVararg) {
            mokkeryErrorAt(
                expression,
                "Expected vararg expression but received ${expression.dumpKotlinLike()}! This should never have happened!"
            )
        }
    ...
    private fun IrBlockBodyBuilder.createDefaultsCallSpec(expression: IrCall): DefaultsCallSpec {
        ...
        val lambda = irLambda(lambdaType, parent) { func ->
            +irCall(calledFunc) {
                ...
                for (index in 1..<expression.arguments.size) {
                    if (expression.arguments[index] == null) {
                        ...
                        arguments[index] = irCall(getFunc) {
                            arguments[0] = list
