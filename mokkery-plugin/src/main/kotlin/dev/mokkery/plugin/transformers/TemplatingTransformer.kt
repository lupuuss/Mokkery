...
class TemplatingTransformer(
    ...
    private val parentFunc: IrDeclarationParent,
    ...
    private val anyMatcher = getFunction(Mokkery.Function._anyMokkeryMatcher)
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        if (receiver == null || cls?.isFinalClass == true) {
            if (expression.usesMatchers) {
                mokkeryErrorAt(expression, "It's only allowed to pass matchers to mock methods!")
            }
        ...
        return declarationIrBuilder(runTemplateFun.symbol) {
            irCall(runTemplateFun) {
                ...
                arguments[4] = irLambda(
                    lambdaType = runTemplateFun.parameters[4].type.makeNotNull(),
                    parent = parentFunc
                ) {
                    createTemplatingLambdaBody(expression, it)
                }
                arguments[5] = if (expression.usesMatchers) {
                    irNull()
                } else {
                    irLambda(
                        lambdaType = runTemplateFun.parameters[5].type.makeNotNull(),
                        parent = parentFunc
                    ) {
    ...
    private fun IrBlockBodyBuilder.createTemplatingLambdaBody(expression: IrCall, function: IrSimpleFunction) {
        ...
        +irReturn(
            irCallMapOf(
                ...
                pairs = calledFunc.nonDispatchParameters.memoryOptimizedMap {
                    ...
                    param to replaceTopTemplatingArg(expression.arguments[it], it)
                }
            )
        )
    }
    ...
    private fun IrBuilderWithScope.replaceTopTemplatingArg(
        frontExpression: IrExpression?,
        frontParam: IrValueParameter
    ): IrExpression {
        return when {
            frontExpression == null -> irCall(anyMatcher) { arguments[0] = irGet(templatingScopeParam) }
            frontExpression.type.isMatcher() -> frontExpression
            frontParam.isVararg -> replaceVararg(frontExpression)
            else -> callEqMatcher(frontExpression)
        }
    }

    private fun IrBuilderWithScope.replaceVararg(expression: IrExpression): IrExpression {
        if (expression !is IrVararg) {
            mokkeryErrorAt(expression, "Expected vararg expression but received ${expression.dumpKotlinLike()}! This should never have happened!")
        }
    ...
    private fun IrBuilderWithScope.compositeVarargMatcher(
        ...
        return irCallConstructor(getClass(Mokkery.Class.CompositeVarArgMatcher).primaryConstructor!!) {
    ...
    private fun IrBuilderWithScope.callEqMatcher(expression: IrExpression) = irCall(eqMatcher) {
        arguments[0] = irGet(templatingScopeParam)
