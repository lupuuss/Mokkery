...
class MatchersInliner(
    ...
    private val matchesFunction = argMatchersScopeClass.functions.find { it.name.asString() == "matches" }
    private val matchesCompositeFunction = argMatchersScopeClass.functions.find { it.name.asString() == "matchesComposite" }
    private val matchersVariables = mutableListOf<IrVariable>()
    private val specialFunctions = mutableSetOf(getFunction(Mokkery.Function.ext))
    private val inlineLiteralsAsMatchersFunc = getFunction(Mokkery.Function.inlineLiteralsAsMatchers)

    override fun visitCall(expression: IrCall): IrExpression {
        if (expression.symbol.owner in specialFunctions) return super.visitCall(expression)
    ...
    private fun IrBuilderWithScope.replaceMatcher(expression: IrExpression): IrExpression {
        ...
        if (matcherFrontFunc == matchesFunction) return expression.arguments[1]!!
        if (matcherFrontFunc == matchesCompositeFunction) return replaceMatchesComposite(expression)
        ...
        val backendFunc = backendFuncCandidates
            ...
            ?: mokkeryErrorAt(expression, "Failed to resolve matcher for ${matcherFrontFunc.dumpKotlinLike()}")
    ...
    private val IrExpression?.isFrontMatcherCall: Boolean
        get() {
            ...
            return owner.parameters.any { it.isArgMatchersScope }
        }

    private val IrValueParameter?.isArgMatchersScope
        get() = this?.type == argMatchersScopeType
