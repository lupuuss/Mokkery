...
class MatchersInliner(
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        return declarationIrBuilder(super.visitCall(expression) as IrCall) {
    ...
    private fun IrBuilderWithScope.replaceMatcher(expression: IrExpression): IrExpression {
        ...
        if (originalMatcherFunction.overrides(matchesFunction)) return expression.arguments[1]!!
        if (originalMatcherFunction.overrides(matchesCompositeFunction)) return replaceMatchesComposite(expression)
