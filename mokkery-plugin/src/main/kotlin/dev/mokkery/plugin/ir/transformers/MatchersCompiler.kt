...
class MatchersCompiler(private val transformerScope: TransformerScope) {
    ...
    private val argMatcherClass = transformerScope.getClass(MokkeryIr.Class.ArgMatcher)
    private val argMatcherCompositeClass = argMatcherClass.nestedClasses.single { it.name.asString() == "Composite" }

    private val matcherAnnotation = transformerScope.getClass(MokkeryIr.Class.Matcher).symbol
    private val matcherScopeType = transformerScope.getClass(MokkeryIr.Class.MokkeryMatcherScope).defaultType
    private val intrinsicsMatchesFunctions = setOf(
        transformerScope.getFunction(MokkeryIr.Function.matches),
        transformerScope.getFunction(MokkeryIr.Function.matchesComposite)
    )
