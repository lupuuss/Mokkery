    ...
    return declarationIrBuilder {
        irBlock {
            ...
            val parentScopeValue = when (extMockParam) {
                null -> {
                    val mokkeryScopeCompanion = getCompanionOf(MokkeryIr.Class.MokkeryScope)
            ...
            val instanceScopeFun = transformer.getFunction(MokkeryIr.Function.createInstanceScope)
            ...
            +irCall(transformer.getFunction(MokkeryIr.Function.initializeInJsFunctionMock)) {
                arguments[0] = irGet(instanceVar)
                arguments[1] = irGet(lambdaVar)
            }
            +irCall(transformer.getFunction(MokkeryIr.Function.invokeInstantiationListener)) {
                arguments[0] = irGet(instanceVar)
                arguments[1] = irGet(lambdaVar)
            }
