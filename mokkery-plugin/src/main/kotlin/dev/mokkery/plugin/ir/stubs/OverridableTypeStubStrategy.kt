...
class OverridableTypeStubStrategy(
    ...
    override fun provide(type: IrType): Stub? {
        ...
        val baseClass = when {
            cls.isInterface -> scope.irBuiltIns.anyClass.owner
    ...
    private fun IrFile.createStubClass(
        ...
        val cls = scope.irFactory.buildClass { this.name = name }
        this.addChild(cls)
        ...
        cls.addConstructor { isPrimary = true }
            .apply {
                body = DeclarationIrBuilder(scope.builder.context, symbol).irBlockBody {
                    +irDelegatingConstructorWithStubs(constructorWithStubs)
                }
            }
        cls.overrideAllOverridableFunctions(scope.plugin, original) { stubFunctionBody(it) }
