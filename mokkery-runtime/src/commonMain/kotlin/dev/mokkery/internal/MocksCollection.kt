...
internal fun MocksCollection(
    vararg values: MokkeryInstanceScope
): MocksCollection = MocksCollection(values.asList())

internal fun MutableMocksCollection(
    vararg values: MokkeryInstanceScope
): MutableMocksCollection = MutableMocksCollection(values.asList())

internal fun MocksCollection(
    values: List<MokkeryInstanceScope>
): MocksCollection = MocksCollectionImpl(values)

internal fun MutableMocksCollection(
    values: List<MokkeryInstanceScope>
): MutableMocksCollection = MocksCollectionImpl(values)
...
internal operator fun MocksCollection.plus(
    other: MocksCollection
): MocksCollection = MutableMocksCollection(scopes + other.scopes)
