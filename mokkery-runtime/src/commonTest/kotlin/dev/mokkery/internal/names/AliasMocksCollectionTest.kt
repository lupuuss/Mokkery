...
class AliasMocksCollectionTest {
    ...
    private val aliasMocks = mocks.withAliasing { it.copy(typeName = it.typeName.removePrefix("package.")) }
    ...
    fun testMapsOriginalToAlias() {
        assertEquals(MockId("foo", 0), aliasMocks.mapOriginalToAlias(mocks.scopes.first().mockId))
    }

    @Test
    fun testMapsAliasToOriginal() {
        assertEquals(MockId("package.foo", 0), aliasMocks.mapAliasToOriginal(MockId("foo", 0)))
    }
}
