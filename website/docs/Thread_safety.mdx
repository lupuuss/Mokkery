...
Mocks created with Mokkery 3 are thread-safe. You can configure them, verify interactions, and invoke their methods
from multiple threads without additional synchronization. Just make sure that any defined matcher/answer does not perform
 any not synchronized side effects. Example below shows this kind of side effect:
...
Mocks created using Mokkery 1 & 2 are generally considered "thread safe" in the context of their usage in a
