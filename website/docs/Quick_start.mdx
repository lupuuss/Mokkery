...
```
If you call a method that has no defined behaviour, it will result in runtime error by default. To learn how to change this locally or globally read [mock mode](../docs/Guides/Mocking#mock_mode) section.
...
By default, `verify` works in [soft](../docs/Guides/Verifying#Soft_modes_family) mode. It checks if each call pattern occured at least once.
...
In conclusion, there is no point of putting duplicated patterns in `verify` with [soft](../docs/Guides/Verifying#Soft_modes_family) mode.
