...
```kotlin
everySuspend { repository.findById(or("1", not("2"))) } returns stubBook()
```
...
```kotlin
everySuspend { repository.findAllById("1", *containsAllElements { it != "2" }, "3") } returns emptyList()
...
```kotlin
everySuspend { repository.findAllById("1", *matches { it.size == 2 }, "3") } returns emptyList()

repository.findAllById("1", "2", "2", "3") // returns empty list
repository.findAllById("1", "5", "5", "3") // returns empty list
repository.findAllById("1", "2", "2", "2", "3") // fails - method not mocked
repository.findAllById("1", "2", "3") // fails - method not mocked
```

Spread operator can be used with composite matchers (e.g. [logical matchers](#logical-matchers)):

```kotlin
everySuspend { repository.findAllById(*any()) } returns listOf(stubBook("1"))
everySuspend { repository.findAllById(*not(containsAllElements { it == "2" })) } returns listOf(stubBook("2"))
...
// for any type
inline fun <reified T> MokkeryMatcherScope.eqAnyOf(
    vararg values: T
): T = matches(
...
everySuspend { repository.findById(capture(slot, not("1"))) } returns stubBook()
...
everySuspend { repository.findByName(query = capture(container), limit = 10) } returns listOf(stubBook())
everySuspend { repository.findByName(query = "Book 3", limit = any()) } returns listOf(stubBook())
...
everySuspend { repository.findAllById(*capture(slot)) } returns listOf(stubBook("1"))
