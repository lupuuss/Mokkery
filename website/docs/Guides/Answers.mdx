...
While using [multiple types mocking](./Mocking_multiple_types) usage of `original` might not be possible, because there are multiple supercalls available for single method.
Instead of `original` use `superOf<T>` with supertype speicifed.
