...
class MatchersUsageInMatcherBuilderTest {
    ...
    fun testForbidsUsingMatchersInNestedFunctions() {
        compileJvm(
            ...
            """.trimIndent()
        ).assertSingleError("Matchers cannot be used in functions declared inside templating functions or matcher declarations, but used in 'fun nested(): Int'. If youre trying to invoke a method with an extension receiver or context parameters, use the `dev.mokkery.templating.ext` or `dev.mokkery.templating.ctx` functions instead.")
    }
    ...
    @Test
    fun testRequiresVarArgMatcherBuilderAnnotationForMatchesWithVarArgMatcher() {
        compileJvm(
            """
                import dev.mokkery.matcher.MokkeryMatcherScope
                import dev.mokkery.matcher.matches
                ...
                @OptIn(dev.mokkery.annotations.DelicateMokkeryApi::class)
                fun MokkeryMatcherScope.matcher(): IntArray = matches(
                    object : VarArgMatcher.Base<Int>() {
                        override fun matchesVarargs(varargs: List<Int>) = true
                    }
                )
            """.trimIndent()
        ).assertSingleError("Matcher that returns vararg matchers must be annotated with @dev.mokkery.annotations.VarArgMatcherBuilder.")
    }

    @Test
    fun testRequiresVarArgMatcherBuilderAnnotationForVarArgMatcherBuilderInvocation() {
        compileJvm(
            """
                import dev.mokkery.matcher.MokkeryMatcherScope
                import dev.mokkery.matcher.varargs.anyVarargsInt
                
                fun MokkeryMatcherScope.matcher(): IntArray = anyVarargsInt()
            """.trimIndent()
        ).assertSingleError("Matcher that returns vararg matchers must be annotated with @dev.mokkery.annotations.VarArgMatcherBuilder.")
    }

    @Test
    fun testRequiresVarArgMatcherWhenBuilderAnnotatedWithVarArgMatcherBuilder() {
        compileJvm(
            """
                import dev.mokkery.matcher.MokkeryMatcherScope
                import dev.mokkery.matcher.any
                ...
                @dev.mokkery.annotations.VarArgMatcherBuilder
                fun MokkeryMatcherScope.matcher(): IntArray {
                    if (kotlin.random.Random.nextBoolean()) return any()
                    else return anyVarargsInt()
                }
            """.trimIndent()
        ).assertSingleError("Matchers annotated with @dev.mokkery.annotations.VarArgMatcherBuilder must return vararg matchers in all returns.")
    }

    @Test
    fun testForbidsReassigningVarArgMatcherToRegularMatcherVariable() {
        tests("anyVarargsInt()", "not(anyVarargsInt())") { expression ->
            compileJvm(
                """
                import dev.mokkery.matcher.MokkeryMatcherScope
                import dev.mokkery.matcher.any
                ...
                fun MokkeryMatcherScope.matcher(): Int {
                    var foo = any<IntArray>()
                    foo = $expression
                    return any()
                }
            """.trimIndent()
            ).assertSingleError("'local var foo: IntArray' is initialized with regular matcher and cannot be reassigned with vararg matcher.")
        }
   }

    @Test
    fun testForbidsReassigningRegularMatcherToVarArgMatcherVariable() {
        tests("anyVarargsInt()", "not(anyVarargsInt())") { expression ->
            compileJvm(
                """
                import dev.mokkery.matcher.MokkeryMatcherScope
            ...
            """.trimIndent()
            ).assertSingleError("'local var foo: IntArray' is initialized with vararg matcher and cannot be reassigned with regular matcher.")

        }
    }

    @Test
    fun testRequiresVarArgMatcherInAllPaths() {
        tests("anyVarargsInt()", "not(anyVarargsInt())") { expression ->
            compileJvm(
                """
                import dev.mokkery.matcher.MokkeryMatcherScope
                ...
                @dev.mokkery.annotations.VarArgMatcherBuilder
                fun MokkeryMatcherScope.matcher(): IntArray {
                    return when (kotlin.random.Random.nextBoolean()) {
                        true -> $expression
                        else -> any<IntArray>()
                    }
                }
            """.trimIndent()
            ).assertSingleError("When vararg matcher is given in one conditional branch, then all other branches must return vararg matchers.")
        }
    }

    @Test
    fun testRequiresCompositeOfOnlyVarArgs() {
        tests("anyVarargsInt()", "not(anyVarargsInt())") { expression ->
            compileJvm(
                """
                import dev.mokkery.matcher.MokkeryMatcherScope
                ...
                @dev.mokkery.annotations.VarArgMatcherBuilder
                fun MokkeryMatcherScope.matcher(): IntArray = and(anyVarargsInt(), any())
            """.trimIndent()
            ).assertSingleError("Given expression must be a vararg matcher or a composite of vararg matchers, because all matchers in a composite must be vararg if any of them is.")
        }
    }
